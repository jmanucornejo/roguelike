...
impl Plugin for ServerClockSyncPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(FixedUpdate, sync_client_time);

        fn sync_client_time(mut server: ResMut<RenetServer>, time: Res<Time>) {
            ...
            for client_id in server.clients_id() {
                ...
                while let Some(message) =
                    server.receive_message(client_id, ClientChannel::SyncTimeRequest)
                {
                    let client_message: ClientSyncMessages =
                        bincode::deserialize(&message).unwrap();
                    match client_message {
                        ClientSyncMessages::Ping { client_time } => {
                            //info!("Got sync time request from {}!", client_id);
                            let sync_time_response =
                                bincode::serialize(&ServerSyncMessages::Pong {
                            ...
                            server.send_message(
                                client_id,
                                ServerChannel::SyncTimeResponse,
                                sync_time_response,
                            );
                        }
                        ClientSyncMessages::SyncTimeRequest { client_time } => {
                            //info!("Got sync time request from {}!", client_id);
                            let sync_time_response =
                                bincode::serialize(&ServerSyncMessages::SyncTimeResponse {
                                    client_time: client_time,
                                    server_time: time.elapsed().as_millis(),
                                })
                                .unwrap();
                        ...
                        ClientSyncMessages::LatencyRequest { client_time } => {
                            ...
                            let sync_time_response =
                                bincode::serialize(&ServerSyncMessages::LatencyResponse {
                                    client_time: client_time,
                                })
                                .unwrap();
                            server.send_message(
