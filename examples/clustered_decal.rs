...
fn main() {
    App::new()
        ...
        .add_systems(Update, handle_ui_interactions::<Selection>)
        .add_systems(
            Update,
            (handle_selection_change, update_radio_buttons)
                .after(handle_ui_interactions::<Selection>),
        )
...
fn spawn_drag_button<'a>(
    ...
    kid.insert((Button, BackgroundColor(Color::BLACK)))
        .with_children(|parent| {
            spawn_ui_text(parent, label, Color::WHITE);
        });
    kid
}
...
fn update_radio_buttons(
    ...
    for (entity, maybe_bg_color, has_text, sender) in &mut widgets {
        ...
        if let Some(mut bg_color) = maybe_bg_color {
            update_ui_radio_button(&mut bg_color, selected);
        }
        if has_text {
